...
import { penjualanService, barangService } from '../api/services';
...
function Penjualan() {
  ...
  const fetchData = async () => {
    try {
      const [penjualanRes, barangRes] = await Promise.all([
        penjualanService.getAll(),
        barangService.getWithStok(),
      ]);
      setPenjualan(penjualanRes.data.data || []);
  ...
  const addDetail = () => {
    ...
    if (selectedBarang && selectedBarang.stok_akhir < parseInt(newDetail.qty)) {
      Swal.fire({
        icon: 'error',
        title: 'Stok Tidak Cukup!',
        text: `Stok tersedia: ${selectedBarang.stok_akhir}`,
      });
      return;
    }

    const hargaJual = newDetail.harga || selectedBarang?.harga_jual || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await penjualanService.create(formData);
      fetchData();
      handleCloseModal();
      Swal.fire({
        icon: 'success',
        title: 'Success!',
        text: 'Penjualan created successfully',
        timer: 2000,
      });
    } catch (error) {
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.response?.data?.message || 'Error creating penjualan',
      });
    }
  };
  ...
  const handleCloseModal = () => {
    setShowModal(false);
    setFormData({ no_faktur: '', customer: '', details: [] });
    setNewDetail({ barang_id: '', qty: '', harga: '' });
  };
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl m-4">
            <h2 className="text-2xl font-bold mb-4">Buat Penjualan Baru</h2>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setNewDetail({ 
                        ...
                        harga: selectedBarang?.harga_jual || ''
                      });
                    }}
